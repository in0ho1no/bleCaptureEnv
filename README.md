...
本手順書の再現性を担保するため以下コマンドを使って、ハッシュ値を確認しておく。
...
:::result
20260602時点のハッシュ値
PS > certutil -hashfile .\nrfutil.exe MD5
MD5 ハッシュ (対象 .\nrfutil.exe):
bd7f4815ddf1d90f4d4496bf294bb45b
...
:::result
20260602時点のハッシュ値
PS > certutil -hashfile .\sniffer_nrf52840dongle_nrf52840_4.1.1.zip MD5
MD5 ハッシュ (対象 .\sniffer_nrf52840dongle_nrf52840_4.1.1.zip):
d23409eb608e479975244816be4dba62
CertUtil: -hashfile コマンドは正常に完了しました。
PS >
:::
...
本プロジェクトはuvによるPython仮想環境を利用できるようにしてある。  
次の手順で必要となるので、以下コマンドを実行しておくこと。
...
1. uf2の[githubページ](https://github.com/microsoft/uf2)から2つのファイルをコピーし、本プロジェクトのtemporary以下に配置する。  
        - `utils/uf2conv.py`  
        - `utils/uf2families.json`

        .\temporary\uf2conv.py
        .\temporary\uf2families.json
...
:::result
20260602時点のハッシュ値
PS > certutil -hashfile .\uf2conv.py MD5
MD5 ハッシュ (対象 .\uf2conv.py):
d6b8913fc3476eb3799a35021bd7c75d
CertUtil: -hashfile コマンドは正常に完了しました。
PS >
:::
...
:::result
20260602時点のハッシュ値
PS > certutil -hashfile .\uf2families.json MD5
MD5 ハッシュ (対象 .\uf2families.json):
b0f9b621f0e61ec45de444954ff41e71
CertUtil: -hashfile コマンドは正常に完了しました。
PS >
:::
...
:::result
20260602時点のハッシュ値
PS > certutil -hashfile .\sniffer_nrf52840dongle_nrf52840_4.1.1.bin MD5
MD5 ハッシュ (対象 .\sniffer_nrf52840dongle_nrf52840_4.1.1.bin):
...
:::result
20260602時点のハッシュ値
PS > certutil -hashfile .\flash.uf2 MD5
MD5 ハッシュ (対象 .\flash.uf2):
e3bec2492f3ee5d86b4dd8a72c9c673a
CertUtil: -hashfile コマンドは正常に完了しました。
PS >
:::
...
:::info
何度かエラーが出るかもしれないが、USBをつないだまま再試行すればうまくいく…ｶﾓ?
:::
...
購入元や製造ロットによっては、出荷時点でSoftDevice S140 v7.3.0がフラッシュ済みの個体が存在する。  
このような個体へ上記手順の`flash.uf2`を書き込もうとしても、書き込み後にデバイスが正常に起動しない。  

:::caution
以下のように`SoftDevice: S140 version 7.3.0`と表示される個体が該当する。
`SoftDevice: not found`と表示される個体は上記手順で問題なく書き込める。
...
`INFO_UF2.TXT`はブートローダーモード時に`XIAO-SENSE`ドライブ内に表示されるファイルから確認できる。
:::

#### 対処手順

正常に書き込みできているOK個体の`CURRENT.UF2`をNG個体へ書き込むことで解消できる。

1. OK個体をダブルリセットしてブートローダーモードに入れる。
1. `XIAO-SENSE`ドライブ内の`CURRENT.UF2`を任意の場所へコピーしておく。
1. OK個体をリセットして通常動作に戻す。
1. NG個体をダブルリセットしてブートローダーモードに入れる。
1. `XIAO-SENSE`ドライブに、2.でコピーした`CURRENT.UF2`をドラッグアンドドロップする。
1. 自動でリセットされ、`nrfutil device list`で認識されれば成功。
...
設定はプロファイルとしてエクスポートしたり、インポートしたりできる。  
今回は予め用意してあるプロファイルをインポートする。  

1. \[編集]-\[設定プロファイル]をクリックする。  
![設定プロファイルダイアログを開く](img/33_profile/01.png)
1. \[インポート]-\[Zipファイルより...]をクリックする  
![インポートボタン](img/33_profile/02.png)
1. 本プロジェクトに同梱の`環境ファイル\WsProfile_ble-sniffer.zip`を選択する。  
![プロファイルの選択](img/33_profile/03.png)
1. 以下表示されれば、インポート成功である。  
![インポート成功](img/33_profile/04.png)
1. インポートしたプロファイルが表示されていることを確認したらOKを押して閉じる。
![確認して閉じる](img/33_profile/05.png)
1. 選択中のプロファイルは、Wiresharkのウィンドウ右下で確認・変更できる。  
![プロファイルの確認](img/33_profile/06.png)

### 列表示の設定例

インポートしたプロファイル以外にも必要な情報がある場合には、設定を変更できる。
...
アドバタイズパケットをアドレスでフィルタリングする | btle.advertising_address | 特になし
